...
const Canvas: React.FC<Props> = ({ projectId, editable, canvasOperations, setCanvasOperations }) => {
    ...
    const saveProject = (projectData: any, editedSounds: any) => {
        ...
        editedSounds.forEach((editedSound: any) => {
            newProjectData.sounds = newProjectData.sounds.map((savedSound: any) => savedSound.id === editedSound.id ? editedSound : savedSound);
        });
    ...
    }, [loadingProject, projectData, canvasOperations, setCanvasOperations, editedSounds, projectsRef]);
